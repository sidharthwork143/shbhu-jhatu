...
async def advantage_spoll_choker(bot, query):
    try:
        ...
        movie = movies[int(movie_)][0] if isinstance(movies[int(movie_)], (tuple, list)) else movies[int(movie_)]
...
for file in all_results:
    ...
    title = file_name.rsplit(".", 1)[0].replace("_", " ").replace(".", " ").strip()
    if title.lower() not in seen:
        seen.add(title.lower())
        alt_titles.append(title)
    if len(alt_titles) >= 15:
        break

scored = [(title, get_match_score(user_query, title)) for title in alt_titles]
scored.sort(key=lambda x: x[1], reverse=True)
top_matches = scored[:5]

if top_matches:
    SPELL_CHECK[query.message.id] = top_matches
    buttons = [
        [InlineKeyboardButton(f"{title} ({score}% match)", callback_data=f"spol#{query.from_user.id}#{i}")]
        for i, (title, score) in enumerate(top_matches)
    ]
    buttons.append([InlineKeyboardButton("❌ Close", callback_data=f"spol#{query.from_user.id}#close_spellcheck")])
    await query.message.reply_text(
        "📺 <b>No exact results found.\nDid you mean:</b>",
